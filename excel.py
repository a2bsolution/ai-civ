...
def extract_xls(df):
	...
	if not item_no_row_index.empty:
		...
		# Stop reading the table when "Inland Charge" or "total" is encountered in the first column
		last_row = None
		for index, row in table_df.iterrows():
			if isinstance(row.iloc[0], str) and any(keyword in row.iloc[0].lower() for keyword in ["inland charge", "total"]):
				table_df = table_df.loc[:index - 1]  # Only keep rows before this point
				last_row = row
				break

		last_row_value = None
		if last_row is not None and isinstance(last_row.iloc[0], str):
			if "total" in last_row.iloc[0].lower():
				last_row_value = last_row.iloc[4]
		elif not table_df.empty:
			last_row_value = table_df.iloc[-1, 4]  # Get the value in column E (index 4) of the last row
		
		first_column_value = table_df.iloc[-1, 0]
		if not table_df.empty:
			if pd.isna(first_column_value) or first_column_value == '':
